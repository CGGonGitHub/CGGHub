...
local Toggle = AutocompleteSection:AddToggle("Autocomplete Normal", {flag = "Toggle_Flag", default = false}, function(bool)
    looping = bool
        for count = previousPos, 146, 1 do
            game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame = workspace.Stages[count].CFrame + Vector3.new(0, 3, 0)
            previousPos = count
            if not looping then game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame = workspace.Stages[1].CFrame + Vector3.new(0,3,0) break end
            task.wait(speeds[_G.Autocompletespeed])
        end
end)
local Slider = AutocompleteSection:AddSlider("Speed of the autocomplete", 1, 7, 1, {toggleable = false, default = false, flag = "Slider_Flag", fireontoggle = true, fireondrag = true, rounded = true}, function(val, bool)
    _G.Autocompletespeed = val
end)
local Button = AutocompleteSection:AddButton("Teleport to Spawn", function()
    game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame = workspace.Stages[1].CFrame + Vector3.new(0,3,0)
end)
local Button = AutocompleteSection:AddButton("Teleport to previous position", function()
	game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame = workspace.Stages[previousPos].CFrame + Vector3.new(0,3,0)
end)
-- Localplayer
